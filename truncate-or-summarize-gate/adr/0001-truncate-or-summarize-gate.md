...
Above the operating-window top (`W_high` = the existing `range_threshold.upper`), cortex no longer truncates unconditionally every step. On every finish where context exceeds `W_high`, a new pure decision — `xcontext.DecideGainAction` — runs the pending `bulk_checkpoint` truncation for real and **truncates** only if it is *effective* (removes ≥ `gain_threshold` of context **and** lands ≤ `W_high`); otherwise it **summarizes immediately** via the existing auto-compact pipeline. The gate is enabled solely by the presence of a new `range_threshold.gain_threshold > 0`, so it rides the same experiment as range-based truncation and ships dark until that field is patched in.

## Why these non-obvious choices

- **Enable via `gain_threshold` inside `range_threshold`, not a new flag.** Range-based truncation is already gated purely by structural presence (no boolean). Reusing that pattern keeps the gate "on only where range-truncation is on," lets the code ship dark, and makes rollout a single experiment field — at the cost of not being able to A/B the gate fully independently of range-truncation (acceptable: the gate is meaningless without it).
...
- **Effectiveness is measured, not estimated (PRD R6), via an estimate-space ratio.** We run the real transform (`xllm.ApplySquash`, pure/no-I/O), take `gain = (est_pre − est_post)/est_pre` as a ratio of the char/4 estimator (bias cancels), and scale the real `S` by the survival ratio for `post_trunc` — keeping it in the same real-token units as `W_high`. A true token count would need an LLM round-trip: synchronous latency + replay nondeterminism, a non-starter under Temporal.
...
- **Naming: "truncation-gain optimization", not "boundary gate".** Per PR review: replaced the abstract "boundary gate" terminology with names that describe *what the optimization measures* — the **gain** from a pending truncation (fraction of context it would reclaim). Surface today: type `GainAction` with values `GainActionProceed/Truncate/Summarize`; types `GainInput` / `GainDecision`; entry point `DecideGainAction`; field `GainThreshold` (config tag `gain_threshold`); policy method `TruncationGainOptimizationActive()`; log line `"truncation gain optimization"` with key `gain`. The file is `core/xcontext/truncation_gain.go`. The word "boundary" survives in this document only where it refers to the *context-window* boundary (a geographic concept), never to the optimization itself.
...
- Gate's S aligns with the squash activity's S (both `contextTokens`), so GainActionTruncate is honoured naturally. No risk of the activity silently overriding the gate.
