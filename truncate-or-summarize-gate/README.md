...
| Doc | Covers | PR |
|---|---|---|
| [design.html](./design.html) | End-to-end design — token bands, the armed state machine, measured (not estimated) effectiveness, dry runs, data flow, file-by-file code changes, failure modes, reporting. | [cortex#1594](https://github.com/emergentbase/cortex/pull/1594) |
| [CONTEXT.md](./CONTEXT.md) | Domain glossary (operating window, W_high/W_low, armed, effective truncation, the two arms). | — |
| [adr/0001-truncate-or-summarize-gate.md](./adr/0001-truncate-or-summarize-gate.md) | The recorded decision + non-obvious trade-offs (gating, forced summarize, no pre-computation, estimate-space math). | — |
...
This adds a **boundary gate**: at the first finish after context exceeds `W_high`, it runs the pending truncation *for real* (pure `xllm.ApplySquash`), measures what it removes, and chooses once — **truncate** if effective (removes ≥ `min_removal_frac` of context **and** lands back ≤ `W_high`), otherwise **summarize immediately**. The trade-off: summarization (a synchronous LLM call) can now fire earlier and is not pre-computed, in exchange for eliminating the futile-truncation churn.
...
- **Loop dispatch**: `core/workflows/elite/flow.go` — calls the gate, dispatches to existing truncate/summarize paths; force-truncate signal honors the gate's decision regardless of the input-only predicate; re-arms on no-op; skips speculative summary pre-computation when active.
...
- **State**: `core/workflows/elite/state.go` — in-memory `boundaryArmed bool` (default true), accessor pair. No DB/BigTable/Redis.
...
- No change for any job without `range_threshold` + `min_removal_frac` (ships dark; rollout is one experiment field).
...
- No new storage. `armed` is in-memory workflow state, recomputed on resume.

## Open items (to validate in prod)

- Initial values `min_removal_frac=0.10`, `W_high=0.325`, `W_low=0.14` are starting points; tune from the `action` split (§12).
